 ...
 *
 * 시트 컬럼(자동 생성): 신청시각 | 이름 | 핸드폰번호 | 기수 | 동의여부 | 공연도움 | 입금일자
 *
 * ※ G열(입금일자)은 IFERROR(VLOOKUP(...), "") 수식으로 채우는 열이며,
 *   이 열에 값이 있는 사람만 "입금 완료 = 신청 완료" 로 카운트합니다.
 *
...
var SHEET_NAME = '신청';
var HEADERS = ['신청시각', '이름', '핸드폰번호', '기수', '동의여부', '공연도움', '입금일자'];

// 어느 버전이 배포됐는지 브라우저로 확인하기 위한 표식.
// 웹앱 주소를 열었을 때 이 값이 안 보이면 옛 코드가 배포된 상태입니다.
var VERSION = 'v5-paid-count';
...
// 입금일자가 들어가는 열(1부터). G열 = 7.
// 이 열에 값이 있으면 "입금 완료 = 신청 완료" 로 봅니다.
// (IFERROR(VLOOKUP(...), "") 수식이라 미입금자는 빈 문자열이 들어옵니다)
var PAID_DATE_COL = 7;

/**
 * "입금 완료" 로 카운트되는 신청 건수.
 *
 * G열(입금일자)에 값이 있는 행만 셉니다. 입금일자는 IFERROR(VLOOKUP(...), "")
 * 수식으로 채워지므로, 아직 입금하지 않은 사람은 빈 문자열("")이 되어 제외됩니다.
 * 시트가 아직 없거나 데이터가 없으면 0.
 */
function countApplications_() {
  ...
  if (lastRow < 2) return 0; // 헤더뿐이거나 빈 시트

  // 2행부터 G열만 읽어 값이 있는 행만 센다.
  var values = sheet.getRange(2, PAID_DATE_COL, lastRow - 1, 1).getValues();
  var count = 0;
  for (var i = 0; i < values.length; i++) {
    // 수식 결과가 "" 이거나 공백뿐이면 미입금으로 본다.
